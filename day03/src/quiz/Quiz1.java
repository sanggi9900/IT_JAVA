...
public class Quiz1 {
	public static void main(String[] args) {
		...
		// 요인 되는 값이 어떤것이 있는지 파악하고, 자료형에 맞춰서 변수를 선언
			String name;		// 문자열 형식의 이름(입력받아야함)
			int age;			// 정수 형식의 나이(입력받아야함)
			String adult;
		...
		// 3) 연산
		// 빈 값이 있는가? adult의 값이 아직 준비되지 않았다
		// age의 값을 이용하여 adult의 값을 계산한다
			adult = (age >=20) ? "성인" : "미성년자";
			
		// 4) 출력
		// 모든값이 준비되었다면 서식을 준비하고 값을 출력한다
			System.out.printf("이름 : %s, 나이 : %d, 성인 : %s" , name, age,adult );
